...
public class EditTimeIntervalDialog extends JDialog {
    ...
    public EditTimeIntervalDialog(TimeInterval timeInterval) {
    ...
    private void startUp() {
        ...
        LocalDateTime start = timeInterval.getStartTime();
        LocalDateTime end = timeInterval.getEndTime();

        int days = LocalDate.of(start.getYear(), start.getMonthValue(), 1).lengthOfMonth();
        DefaultComboBoxModel<Integer> model = new DefaultComboBoxModel<>();
        addDays(days, model);
        dayBeginComboBox.setModel(model);
        days = LocalDate.of(end.getYear(), end.getMonthValue(), 1).lengthOfMonth();
        ...
        // Date
        dayBeginComboBox.setSelectedIndex(start.getDayOfMonth() - 1);
        monthBeginComboBox.setSelectedIndex(start.getMonthValue() - 1);
        yearBeginComboBox.setSelectedIndex(start.getYear() - 2000);

        // Time
        hoursBeginSpinner.setModel(new SpinnerNumberModel(start.getHour(), 0, 23, 1));
        minutesBeginSpinner.setModel(new SpinnerNumberModel(start.getMinute(), 0, 59, 1));
        secondsBeginSpinner.setModel(new SpinnerNumberModel(start.getSecond(), 0, 59, 1));

        // End

        // Date
        dayEndComboBox.setSelectedIndex(end.getDayOfMonth() - 1);
        monthEndComboBox.setSelectedIndex(end.getMonthValue() - 1);
        yeahEndComboBox.setSelectedIndex(end.getYear() - 2000);

        // Time
        hoursEndSpinner.setModel(new SpinnerNumberModel(end.getHour(), 0, 23, 1));
        minutesEndSpinner.setModel(new SpinnerNumberModel(end.getMinute(), 0, 59, 1));
        secondsEndSpinner.setModel(new SpinnerNumberModel(end.getSecond(), 0, 59, 1));

        // Duration
        long duration = timeInterval.getDuration().toSeconds();
        durationLabel.setText("Duration: " + String.format("%02d:%02d:%02d", duration / 3600, (duration % 3600) / 60, (duration % 60)));
    }
}
